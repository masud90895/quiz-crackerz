...
const QuizContainer = () => {
  ...
  useEffect(() => {
    const getRightData = JSON.parse(localStorage.getItem('rightAnswer'))
    if (getRightData) {
      setRight(getRightData);
    }
  },[])
  useEffect(() => {
    const getWrongData = JSON.parse(localStorage.getItem('wrongAnswer'))
    if (getWrongData) {
      setwrong(getWrongData);
    }
  },[])
